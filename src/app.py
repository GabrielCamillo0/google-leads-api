...
# Banco de dados de usuários e planos (Simulado)
USUARIOS = {
    "usuario_gratis": {"plano": "gratuito", "buscas_restantes": 5, "max_leads": 10},
    "usuario_premium": {"plano": "premium", "buscas_restantes": 99999, "max_leads": 99999}
}
...
def autenticar_usuario(chave):
    return USUARIOS.get(chave, None)

def buscar_leads(usuario, site, keyword):
    """Faz uma busca no Google usando a Custom Search API para capturar nome, e-mail, telefone e link."""
    if usuario["buscas_restantes"] <= 0:
        messagebox.showwarning("Limite Atingido", "Você atingiu o limite de buscas diárias. Faça upgrade para um plano premium.")
        return []

    usuario["buscas_restantes"] -= 1  # Reduz uma busca disponível
    ...
    params = {"q": QUERY, "key": API_KEY, "cx": CX_ID, "num": 10}

    leads = []
    start = 1  # Paginação inicial
    max_results = usuario["max_leads"]  # Limite de acordo com o plano
...
def salvar_em_planilha(resultados, usuario):
    """Salva os resultados em um arquivo Excel (Somente premium)."""
    if not resultados:
        messagebox.showwarning("Aviso", "Nenhum resultado para salvar.")
        return

    if usuario["plano"] == "gratuito":
        messagebox.showwarning("Plano Limitado", "Apenas usuários premium podem salvar em Excel.")
        return
...
def iniciar_busca():
    ...
    chave_usuario = entrada_chave.get().strip()

    usuario = autenticar_usuario(chave_usuario)
    if not usuario:
        messagebox.showwarning("Acesso Negado", "Chave de acesso inválida. Obtenha uma chave premium em nosso site.")
        return
    ...
    resultados = buscar_leads(usuario, site, keyword)
    if resultados:
...
# Interface Gráfica
app = tk.Tk()
app.title("Google Leads Scraper - Versão Monetizada")

# Entrada de chave de acesso
tk.Label(app, text="Chave de Acesso:").pack(pady=5)
entrada_chave = tk.Entry(app, width=50)
entrada_chave.pack(pady=5)

# Entrada de site
tk.Label(app, text="Site para buscar:").pack(pady=5)
entrada_site = tk.Entry(app, width=50)
entrada_site.insert(0, "instagram.com")
...
entrada_keyword.insert(0, "roupas")
...
btn_salvar = tk.Button(app, text="Salvar em Excel", command=lambda: salvar_em_planilha(buscar_leads(autenticar_usuario(entrada_chave.get()), entrada_site.get(), entrada_keyword.get()), autenticar_usuario(entrada_chave.get())))
